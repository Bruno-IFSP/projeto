...
if (isset($_GET['idprova'])) {
    ...
    if ($resultado && mysqli_num_rows($resultado) > 0) {
        ...
        while ($pergunta = mysqli_fetch_assoc($resultadoPerguntas)) {
            ...
            echo "<input type='hidden' name='pergunta_alternativa[{$pergunta['idpergunta']}]' value=''>";
            while ($alternativa = mysqli_fetch_assoc($resultadoAlternativas)) {
                echo "<input type='radio' name='pergunta{$pergunta['idpergunta']}' value='{$alternativa['idalternativa']}'> {$alternativa['textoalternativa']}<br>";
            }
            echo "</div>"; 
            echo "</div>"; 
        }
        ...
        while ($pergunta = mysqli_fetch_assoc($resultadoPerguntas)) {
            ...
            while ($alternativa = mysqli_fetch_assoc($resultadoAlternativas)) {
                ...
                echo "<input type='radio' name='respostas[$idPergunta]' value='$idAlternativa'> {$alternativa['textoalternativa']}<br>";
            }
            echo "</div>"; 
            echo "</div>"; 
        }
